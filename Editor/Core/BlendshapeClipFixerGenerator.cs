...
namespace BlendshapeClipFixer.Editor
{
    public static class BlendshapeClipFixerGenerator
    {
            ...
            foreach (var b in bindings)
            {
                ...
                if (targetNames == null || targetNames.Count == 0)
                {
                    ...
                    else
                    {
                        unresolved++;
                        if (verbose)
                            BlendshapeClipFixerLog.Info($"Unresolved shape in clip='{clip.name}': '{oldShape}' (path='{b.path}')");
                        continue;
                    }
                }
                ...
                if (targetNames.Count == 0)
                {
                    unresolved++;
                    if (verbose)
                        BlendshapeClipFixerLog.Info($"Mapped-to-missing shape in clip='{clip.name}': '{oldShape}'");
                    continue;
                }
        ...
        private static void Log(string msg) => BlendshapeClipFixerLog.Info(msg);
    }
}
