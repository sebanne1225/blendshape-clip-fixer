...
namespace BlendshapeClipFixer.Editor
{
    public sealed class BlendshapeClipFixerWindow : EditorWindow
    {
        ...
        public static void Open()
        {
            ...
            w.titleContent = new GUIContent("BlendShape Clip Fixer");
        ...
        private void OnGUI()
        {
            ...
            using (new EditorGUILayout.VerticalScope("box"))
            {
                DrawSectionHeader(
                    "Inputs",
                    "入力",
                    "Select the source controller and the renderer used to resolve BlendShape mappings.",
                    "BlendShape の対応付けに使う元コントローラーと Renderer を設定します。");
                DrawInputFields();
            }
            ...
            using (new EditorGUILayout.VerticalScope("box"))
            {
                DrawSectionHeader(
                    "Options",
                    "オプション",
                    "Adjust path fix, output, and logging behavior before scanning or generating.",
                    "スキャンや生成の前に、パス修正、出力、ログの挙動を調整します。");
                DrawOptions();
            }
            ...
            using (new EditorGUILayout.VerticalScope("box"))
            {
                DrawSectionHeader(
                    "Scan and Review",
                    "スキャンと確認",
                    "Scan the controller, review results, then confirm mappings and bindings before generation.",
                    "コントローラーをスキャンし、結果を確認してからマッピングとバインディングを確認します。");
            ...
            using (new EditorGUILayout.VerticalScope("box"))
            {
                DrawSectionHeader(
                    "Generate",
                    "生成",
                    "Create a fixed controller and clips using the settings and mappings confirmed above.",
                    "上で確認した設定とマッピングを使って、修正版コントローラーとクリップを生成します。");

                GUI.enabled = lastScan != null;
                if (GUILayout.Button(L("Generate Fixed Assets", "修正アセットを生成"), GUILayout.Height(30)))
                    Generate();
                GUI.enabled = true;
            }
        ...
        private void DrawInputFields()
        {
            ...
            targetAvatarRoot = (GameObject)EditorGUILayout.ObjectField(
                new GUIContent(L("Target Avatar Root (optional)", "対象アバタールート（任意）"), L("Used to compute the relative path when path fix is enabled.", "パス修正が有効な時に、対象 Renderer の相対パス計算に使用します。")),
                targetAvatarRoot, typeof(GameObject), true);

            targetRenderer = (SkinnedMeshRenderer)EditorGUILayout.ObjectField(
                new GUIContent(L("Target SkinnedMeshRenderer", "対象 SkinnedMeshRenderer"), L("Renderer that has the correct BlendShapes (usually a face mesh).", "正しい BlendShape を持つ Renderer です。通常は顔メッシュを指定します。")),
                targetRenderer, typeof(SkinnedMeshRenderer), true);

            if (targetRenderer == null)
                EditorGUILayout.HelpBox(L("Set Target SkinnedMeshRenderer to enable editing of the Missing / Replace Map and show mapping suggestions.", "対象 SkinnedMeshRenderer を設定すると、Missing / 置換マップを編集でき、マッピング候補も表示されます。"), MessageType.Info);
        ...
        private void DrawOptions()
        {
            forceAllBlendshapePathsToTargetRenderer = EditorGUILayout.ToggleLeft(
                new GUIContent(L("Force all BlendShape curves to target renderer path", "すべての BlendShape カーブを対象 Renderer パスへ寄せる"),
                    L("If enabled, all BlendShape curves are re-bound to the selected target renderer path relative to Avatar Root. This helps resolve path mismatches.", "有効にすると、すべての BlendShape カーブを Avatar Root 基準の対象 Renderer パスへ再バインドします。パス不一致の修正に使います。")),
            ...
            if (forceAllBlendshapePathsToTargetRenderer && targetAvatarRoot == null)
                EditorGUILayout.HelpBox(L("Path fix is enabled, but Avatar Root is not set. BlendShape names can still be fixed, but renderer path adjustment is unavailable.", "パス修正は有効ですが Avatar Root が未設定です。BlendShape 名の修正はできますが、Renderer パスの調整は行えません。"), MessageType.Warning);
        }

        private void DrawSectionHeader(string titleEn, string titleJa, string descriptionEn, string descriptionJa)
        {
            EditorGUILayout.LabelField(L(titleEn, titleJa), EditorStyles.boldLabel);
            EditorGUILayout.LabelField(L(descriptionEn, descriptionJa), EditorStyles.wordWrappedMiniLabel);
        ...
        private void DrawScanSummary()
        {
            ...
            if (lastScan == null)
            {
                EditorGUILayout.HelpBox(L("Press 'Scan Controller' to list clips and missing BlendShapes.", "「コントローラーをスキャン」を押すと、クリップと missing な BlendShape を一覧表示します。"), MessageType.Info);
                return;
            }

            EditorGUILayout.LabelField($"{L("Controller", "コントローラー")}: {lastScan.ControllerName}");
            EditorGUILayout.LabelField($"{L("Clips", "クリップ")}: {lastScan.Clips.Count}   {L("BlendShape Bindings", "BlendShape バインディング")}: {lastScan.TotalBlendshapeBindings}");
            EditorGUILayout.LabelField($"{L("Unique BlendShapes", "ユニーク BlendShape")}: {lastScan.UniqueBlendshapeNames.Count}   {L("Missing BlendShapes", "missing BlendShape")}: {lastScan.MissingBlendshapeNames.Count}");
            EditorGUILayout.LabelField($"{L("Path Mismatches (vs Target Renderer)", "パス不一致（対象 Renderer 基準）")}: {lastScan.PathMismatchCount}");
            EditorGUILayout.LabelField($"{L("Target Renderer Path", "対象 Renderer パス")}: {(string.IsNullOrEmpty(lastScan.TargetRendererPath) ? L("(not available)", "（未設定）") : lastScan.TargetRendererPath)}");
            ...
            if (lastScan.MissingBlendshapeNames.Count > 0 && targetRenderer != null)
                EditorGUILayout.HelpBox(L("Fill the Missing / Replace Map below. Unresolved entries are left unchanged and reported in the log.", "下の Missing / Replace Map を設定してください。未解決の項目は変更せず、そのままログに出力します。"), MessageType.Warning);
        }
        ...
        private void DrawMappingUI()
        {
            ...
            {
                EditorGUILayout.HelpBox(L("Set Target SkinnedMeshRenderer to edit this map.", "このマップを編集するには対象 SkinnedMeshRenderer を設定してください。"), MessageType.Info);
                return;
            }
            ...
            {
                EditorGUILayout.HelpBox(L("No missing BlendShape names were detected for the selected target renderer.", "選択した対象 Renderer に対して missing な BlendShape 名は検出されませんでした。"), MessageType.Info);
                return;
            }
            ...
            if (TryGetFocusedClip(out var clip))
            {
                ...
                {
                    EditorGUILayout.HelpBox(L("The focused clip has no missing BlendShape names.", "集中対象クリップには missing な BlendShape 名がありません。"), MessageType.Info);
                    return;
                }
            }
        ...
        private void DrawIssuesUI()
        {
            ...
            using (new EditorGUILayout.VerticalScope("box"))
            {
                EditorGUILayout.LabelField(L("Preview list of BlendShape bindings found in clips. 'Missing' means the shape is not found on the selected renderer mesh or the binding path does not match the target renderer path.", "クリップ内で検出した BlendShape バインディングのプレビューです。'Missing' は、対象 Renderer メッシュに形状が存在しないか、バインディングのパスが対象 Renderer パスと一致しないことを示します。"));
                EditorGUILayout.Space(4);
                ...
                foreach (var issue in issues)
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        EditorGUILayout.LabelField(FormatIssueReason(issue.Reason), GUILayout.Width(160));
                    }
                    shown++;
                }
            }
        ...
        private void Scan()
        {
            ...
            {
                EditorUtility.DisplayDialog(L("Missing input", "入力不足"), L("Source Controller is required.", "Source Controller を設定してください。"), "OK");
                return;
            }
            ...
            BlendshapeClipFixerLog.Info($"Scan: clips={lastScan.Clips.Count} blendshapeBindings={lastScan.TotalBlendshapeBindings} uniqueShapes={lastScan.UniqueBlendshapeNames.Count} missingShapes={lastScan.MissingBlendshapeNames.Count} pathMismatches={lastScan.PathMismatchCount}");
            SavePersistedState();
        }
        ...
        private void Generate()
        {
            ...
            {
                EditorUtility.DisplayDialog(L("Invalid output folder", "出力フォルダが無効"), L("Output Folder must be under Assets/.", "出力フォルダは Assets/ 配下である必要があります。"), "OK");
                return;
            }
            ...
            {
                EditorUtility.DisplayDialog(L("Invalid controller", "コントローラーが無効"), L("Could not find the selected AnimatorController asset path.", "選択した AnimatorController のアセットパスが見つかりません。"), "OK");
                return;
            }
        ...
        private void AutoMapByKeywordPrefixContains(string[] targetShapes)
        {
            ...
            if (string.IsNullOrWhiteSpace(mappingSearchKeyword))
            {
                EditorUtility.DisplayDialog(L("Keyword required", "キーワードが必要です"), L("Enter a search keyword first.", "先に「検索キーワード」を入力してください。"), "OK");
                return;
            }
        ...
        private string FormatIssueReason(string reason)
        {
            return reason switch
            {
                "MissingShape" => L("Missing BlendShape", "不足 BlendShape"),
                "PathMismatch" => L("Path Mismatch", "パス不一致"),
                _ => reason
            };
        }
